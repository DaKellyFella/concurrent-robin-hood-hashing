...
template <class Allocator, class MemReclaimer> class HarrisKCAS {
public:
...
public:
  ...
  private:
    ...
    KCASDescriptor(const std::size_t descriptor_size)
        : state(0), m_descriptor_size(descriptor_size),
          m_status(KCASDescriptorStatus::UNDECIDED), m_num_entries(0),
  ...
  HarrisKCAS(const std::size_t threads, MemReclaimer *reclaimer)
